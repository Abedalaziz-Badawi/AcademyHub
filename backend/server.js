...
const bootUsername=String(process.env.ADMIN_USERNAME||'').trim();
const bootPassword=String(process.env.ADMIN_PASSWORD||'');
const authSecret=String(process.env.AUTH_SECRET||'');
const secretConfigured=authSecret.length>=32;
const createToken=(username,role)=>{const payload=Buffer.from(JSON.stringify({username,role,exp:Date.now()+1000*60*60*8})).toString('base64url');const signature=crypto.createHmac('sha256',authSecret).update(payload).digest('base64url');return `${payload}.${signature}`};
...
app.post('/api/auth/login',async(req,res)=>{
 if(!secretConfigured)return res.status(503).json({error:'Login is not configured. Set AUTH_SECRET in .env (32+ characters).'});
 const {username,password}=req.body||{};
 try{
  const r=await q('SELECT * FROM staff_users WHERE username=$1',[String(username||'').trim()]);
  const u=r.rows[0];
  ...
  return res.json({token:createToken(u.username,u.role),user:{username:u.username,name:u.name,role:u.role}});
 }catch(e){console.error(e);return res.status(500).json({error:'Server error'})}
});
const requireAuth=(req,res,next)=>{const token=req.headers.authorization?.replace(/^Bearer\s+/i,'');if(!secretConfigured)return res.status(503).json({error:'Login is not configured'});const data=readToken(token);if(!data)return res.status(401).json({error:'Please sign in to continue'});req.authUser=data;next()};
app.all('/api/data',requireAuth,async(req,res)=>{
 const type=req.query.type||'dashboard';
 if(type==='staff'&&req.authUser.role!=='Admin')return res.status(403).json({error:'Admin access required'});
 try{
  if(req.method==='GET'){
   if(type==='dashboard'){
   ...
   if(type==='staff'){const result=await q('SELECT id,username,name,role,created_at FROM staff_users ORDER BY name');return res.json({staff:result.rows})}
  ...
  if(req.method==='POST'){
   ...
   if(type==='staff'){
    const username=clean(b.username),name=clean(b.name),role=b.role==='Admin'?'Admin':'Staff';
    if(!username||!b.password||!name)return res.status(400).json({error:'Name, username and password are required'});
    try{
     const hash=await bcrypt.hash(String(b.password),10);
     const r=await q('INSERT INTO staff_users(username,password_hash,name,role) VALUES($1,$2,$3,$4) RETURNING id,username,name,role,created_at',[username,hash,name,role]);
     return res.status(201).json({staff:r.rows[0]});
    }catch(e){if(e.code==='23505')return res.status(409).json({error:'That username is already taken'});throw e}
   }
  }
  if(req.method==='PUT'){
   ...
   if(type==='staff'){
    const id=Number(b.id);if(!id)return res.status(400).json({error:'Invalid staff id'});
    const role=b.role==='Admin'?'Admin':'Staff';
    if(b.password){const hash=await bcrypt.hash(String(b.password),10);await q('UPDATE staff_users SET name=$1,role=$2,password_hash=$3 WHERE id=$4',[clean(b.name),role,hash,id])}
    else{await q('UPDATE staff_users SET name=$1,role=$2 WHERE id=$3',[clean(b.name),role,id])}
    return res.json({ok:true});
   }
  }
  if(req.method==='DELETE'){
   if(type==='player'){await q("DELETE FROM players WHERE id=$1",[Number(req.query.id)]);return res.json({ok:true})}
   if(type==='staff'){
    const id=Number(req.query.id);
    const target=await q('SELECT username,role FROM staff_users WHERE id=$1',[id]);
    if(!target.rows.length)return res.status(404).json({error:'Staff account not found'});
    if(target.rows[0].username===req.authUser.username)return res.status(400).json({error:"You can't delete your own account while logged in"});
    if(target.rows[0].role==='Admin'){const admins=await q("SELECT count(*)::int c FROM staff_users WHERE role='Admin'");if(admins.rows[0].c<=1)return res.status(400).json({error:'Cannot delete the last remaining admin account'})}
    await q('DELETE FROM staff_users WHERE id=$1',[id]);
    return res.json({ok:true});
   }
  }
...
async function ensureBootstrapAdmin(){
 try{
  const r=await q('SELECT count(*)::int c FROM staff_users');
  if(r.rows[0].c>0)return;
  if(!bootUsername||!bootPassword||!secretConfigured){console.log('No staff accounts yet. Set ADMIN_USERNAME, ADMIN_PASSWORD and AUTH_SECRET to bootstrap the first admin account.');return}
  const hash=await bcrypt.hash(bootPassword,10);
  await q('INSERT INTO staff_users(username,password_hash,name,role) VALUES($1,$2,$3,$4) ON CONFLICT(username) DO NOTHING',[bootUsername,hash,'Academy Admin','Admin']);
  console.log('Bootstrap admin account created:',bootUsername);
 }catch(e){console.error('Bootstrap admin check failed (has npm run migrate been run yet?):',e.message)}
}
ensureBootstrapAdmin().finally(()=>app.listen(PORT,()=>console.log(`AcademyHub running on http://localhost:${PORT}`)));
